...
static void pad_added_handler(GstElement *src, GstPad *new_pad, PipelineElements *eles) {
    GstPad *audiosinkpad = gst_element_get_static_pad(eles->audioconvert, "sink");
    GstPad *videosinkpad = gst_element_get_static_pad(eles->videoconvert, "sink");

    g_print("new pad type: %s.%s\n", GST_ELEMENT_NAME(src), GST_PAD_NAME(new_pad));

    if (gst_pad_is_linked(audiosinkpad) && gst_pad_is_linked(videosinkpad)) {
        goto exit;
    }
    ...
    if (GST_PAD_LINK_FAILED(ret)) {
        g_printerr("dynamic pad linking failed. pad type = %s\n", pad_type);
        goto exit;
    } else {
        g_print("dynamic pad linking done. %s -> %s\n", pad_type, GST_PAD_NAME( g_str_has_prefix(pad_type, "audio/x-raw") ? audiosinkpad : videosinkpad));
    }
